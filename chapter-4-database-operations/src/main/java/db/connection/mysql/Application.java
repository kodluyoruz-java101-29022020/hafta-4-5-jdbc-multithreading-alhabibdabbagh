...
public class Application {
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			case 6:
				...
				break;
			case 7:
				listActiveManagers(managerService);// aktiv calisan
				System.out.println();
				break;
			case 8:
				listDepartments(departmentService);// butun departments
				System.out.println();
				break;
			default:
				break;
	...
	public static void listActiveManagers(ManagerService managerService) {
		...
		List<Manager> Managers = managerService.findAllActiveManagers();// application => service => DAO => database
																		// sonra donduruyor
		Iterator<Manager> iterator = Managers.iterator();// iterator

		while (iterator.hasNext()) {// bosolmadikca girecektir
			Manager manager = iterator.next();// gosterlen bilgi burda kayit etsin
			System.out.println(manager.getDepartmentName() + " " + manager.getEmployee()); // yazdirma
		}
	...
	public static void listDepartments(DepartmentService departmentService) {
		...
		while (iterator.hasNext()) {
			...
			System.out.println(department.getName() + " " + department.getDeptNo());
		}

//		for (Department arr : departments) {// farkli yazdirma icin 
//			System.out.println(arr.getName()+" "+arr.getDeptNo());
//		}
	}
